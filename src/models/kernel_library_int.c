...
void Transpose(const int8_t *input, int8_t *output, int32_t size, int32_t rank,
  ...
  /*
   * 性能优化：原实现对每个输出元素做 rank 次取模(%)和除法(/)，
   * Cortex-M85 没有硬件除法，代价很高。这里改用“混合进制里程表”
   * 增量进位，除法只在初始化时算一次，内层只剩比较/加减。
   * rank 本工程恒为 5，d[8]/radix[8] 足够；超出 8 时回退到通用实现。
   * 注意：此文件由 RUHMI 生成，重新转换模型会覆盖本函数，需重新应用此优化。
   */
  if (rank > 8) {
    int32_t out_ind = 0;
    while (out_ind < size) {
      ...
      for (int i = 0; i < rank; i++) {
        idx += strides[i] * (int32_t)((out_ind % next_dim_sizes[i]) / dim_sizes[i]);
      }
  ...
  for (int i = 0; i < rank; i++) {
    d[i] = 0;
    radix[i] = next_dim_sizes[i] / dim_sizes[i];
  }

  for (int32_t out_ind = 0; out_ind < size; out_ind++) {
