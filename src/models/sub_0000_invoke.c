...
__attribute__((aligned(16))) uint8_t sub_0000_arena[1228800] BSP_PLACE_IN_SECTION(".ram_nocache");
...
int sub_0000_invoke(bool clean_outputs) {
  ...
  // Buffer sub_0000_model with size 437056 and address: 4294967295
  ...
  // Buffer output_tensor_0 with size 12000 and address: 21600
  if (clean_outputs) {
    memset(sub_0000_arena + 21600, 0, 12000);
  }
  base_addrs[4] = (uint64_t)(uintptr_t) (sub_0000_arena+21600);
  base_addrs_size[4] = 12000;

  // Buffer output_tensor_1 with size 3000 and address: 0
  if (clean_outputs) {
    memset(sub_0000_arena + 0, 0, 3000);
  }
  base_addrs[5] = (uint64_t)(uintptr_t) (sub_0000_arena+0);
  base_addrs_size[5] = 3000;
