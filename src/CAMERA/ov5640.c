...
//    sccb_3_phase_write(OV5640_SCCB_ADDR, reg, dat);
    uint8_t temp[4] = {0};
    temp[0] = dat;
    R_IIC_MASTER_SlaveAddressSet(&g_i2c_master0_ctrl, OV5640_I2C_SLAVE_ADDR, I2C_MASTER_ADDR_MODE_7BIT);
 ...
 * @retval      无
 */
/* Configure OV5640 for DVP VGA 15 fps RGB565 timing. */
void ov5640_config_dvp_vga_rgb565_15fps(void)
{
    static const ov5640_reg_cfg_t regs[] =
    {
        {0x3008, 0x42},
        {0x3017, 0xFF},
        {0x3018, 0xFF},
...
//        {0x3821, 0x07},
        ...
        {0x3824, 0x02},
        {0x5001, 0xA3},
        /* 0x3503: bit0=手动曝光 bit1=手动增益。原来 0x00 = AEC/AGC 全自动——
         * 运行中自动拉长曝光时间导致帧率从 ~16fps 漂移到 3.6fps（实测）。
         * 0x03 关闭自动曝光/增益，帧率固定（比赛演示需要稳定帧率）。 */
        {0x3503, 0x03},
...
uint8_t ov5640_init_qqvga_rgb565_slow(void)
{
    ...
    static const ov5640_reg_delay_t friend_qqvga_regs[] =
    {
#include "ov5640_friend_qqvga_rgb565.inc"
    };

    for (uint32_t i = 0; i < (sizeof(friend_qqvga_regs) / sizeof(friend_qqvga_regs[0])); i++)
    {
        ov5640_write_reg(friend_qqvga_regs[i].reg, friend_qqvga_regs[i].dat);
        if (friend_qqvga_regs[i].delay > 0U)
        {
            delay_ms(friend_qqvga_regs[i].delay);
        }
    }
...
uint8_t ov5640_set_output_format(ov5640_output_format_t format)
{
    ...
    switch (format)
    {
        case OV5640_OUTPUT_FORMAT_RGB565:
        {
            ...
            /* Match the DVP VSYNC polarity used by the vendor-style init table. */
            ov5640_write_reg(0x4740, 0x21);
