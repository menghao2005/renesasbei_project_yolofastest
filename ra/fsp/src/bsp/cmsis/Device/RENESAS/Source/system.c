...
void SystemInit(void)
{
    ...
    *p_main_stack_top = BSP_TZ_STACK_SEAL_VALUE;
    ...
    if ((MEMSYSCTL->MSCR & MEMSYSCTL_MSCR_ECCEN_Msk) && (SCB->VTOR == BSP_PRV_VTOR_FIRST_PROJECT))
    {
        const size_t itcm_num_doublewords = (1U
                << (((MEMSYSCTL->ITCMCR & MEMSYSCTL_ITCMCR_SZ_Msk) >> MEMSYSCTL_ITCMCR_SZ_Pos) + 9U))
                / sizeof(uint64_t);
        const size_t dtcm_num_doublewords = (1U
                << (((MEMSYSCTL->DTCMCR & MEMSYSCTL_DTCMCR_SZ_Msk) >> MEMSYSCTL_DTCMCR_SZ_Pos) + 9U))
                / sizeof(uint64_t);
